      ...
      for(var j=0; j < optionNames.length; j++){
        ...
        if (typeof data.options_with_values != 'undefined' && data.options_with_values.length > 0) {
          ...
          if (swatchArr.length > 0) {
            ...
            for (var sIndex = 0; sIndex < countSwatch; sIndex++) {
              swatchName = swatchValues[sIndex].title;
              swatchVariant = data['variants'][sIndex];
              sImageIndex = swatchValues[sIndex].image || '';
              if (sImageIndex != '') {
                if (data.hasOwnProperty('split_product')) {
                  dataImgSrc = Utils.optimizeImage(data.original_images[sImageIndex - 1]['src'], dataImgSize + 'x') + ' ' + dataImgSize + 'w';
                } else {
                  dataImgSrc = Utils.optimizeImage(data.images[sImageIndex - 1], dataImgSize + 'x') + ' ' + dataImgSize + 'w';
                }
              }
